...
\cite{Annunziato2008}, requiring a huge degree of compaction to fit
...
\caption{\label{fig:nucleosome-xray-structure}Structure of the nucleosome}


X Ray Structure of a Nucleosome core particle at a resolution of 1.9Å.
DNA can be seen wrapped around the core histones. PDB structure 1KX5
\cite{Davey2002}.\end{wrapfigure}%
...
\caption{\label{fig:Histone-mod-dashboard}Histone modification associations
in chromatin}
\end{figure}
...
\emph{cis} organisation of Hox genes, gene clusters, gene rich areas
and gene deserts


\section{Chromosome Territories}

Chromosome painting, what they are, not much more. (talk about organisation
of CTs later). CTs being close to the nuclear periphery, CT intermingling,
being buried in CTs or on the surface.

\rule[0.5ex]{1\columnwidth}{1pt}

Interphase chromosomes are organised into discrete regions, known
as chromosome territories (\nomenclature{CT}{Chromosome territory}CTs)
\cite{Cremer2001}. Chromosome territories are not randomly positioned,
and territory position has been shown to be conserved through evolution
\cite{Tanabe2002}. Fluorescence \emph{in-situ} hybridisation (FISH)
studies have demonstrated large scale rearrangements of CTs with changes
in cell morphology, such as in chicken hematopoietic cell differentiation
. Studies of CT localisation within multiple mouse cell types have
shown that CT organisation can be tissue specific \cite{Parada2004b}.
Certain chromosomes have preferred neighbours in some tissue types,
for example chromosomes 12 and 15 are frequently close in mouse lymphocytes
but not mouse hepatocytes \cite{Parada2004b}. This is relevant as
translocations between these chromosomes are common in lymphomas but
not in hepatomas. The positions adopted by chromosome territories
within the nucleus appears to be related to the sequence encoded by
each chromosome \textendash{} those near the centre of the nucleus
tend to be gene-rich, often replicate early and are frequently relatively
small, whilst the opposite is true for chromosomes located near the
nuclear periphery \cite{Sun2000,Croft1999}.
...
Transcription factories in detail as distinct sub-compartments. Transcription
takes place in transcription factories. Stochastic bursting transcription.
...
seen within the nucleus which did not form in the prescence of the
...
light microscopy can visualize transcription through labelling BrUTP
...
\caption{\label{fig:transcription-factories}Nascent RNA and transcription
factories.}


(A) Transcription foci in HeLa cells, visualised with labelled Br-UTP
in 100 nm cryosections. Nascent RNA (green) is concentrated in punctate
foci. (B) Model for a nucleolar factory, showing a transcript with
multiple polymerases generating a crescent shaped foci. (C) Model
...
Taken from Cook \emph{et al.}, Science (2009) \cite{Cook1999}.\end{wrapfigure}%
...
transciption factories in HeLa cells calculating the number of active
...
II complexes \cite{Jackson1998}. The number of transcription factories
varies a great deal amongst cell types, but the observation that genes
colocalise within transcription factories has been confirmed by a
number of different techniques \cite{Osborne2004}.

\begin{comment}
Osborne et al. used DNA-FISH (fluorescence in-situ hybridisation),
RNA-FISH, immunofluorescence and 3C (chromosome conformation capture)
to show that genes situated within transcription factories are actively
transcribed, whereas those outside are not {[}2{]}. Transcription
was shown to be discontinuous, with the frequency of nascent RNA transcription
foci related to cellular mRNA concentrations, suggesting that transcription
occurs in bursts. Multiple genes both in cis and in trans were seen
to dynamically colocalise in transcription factories, confirming earlier
predictions that genes must share transcription factories {[}2, 18{]}.
Osborne et al. suggested that the nuclear architecture may be in part
organised by transcription.

The transcription factory model provides answers to several theoretical
problems regarding transcription: a static polymerase with nearby
topoisomerase activity would allow nascent RNA to extrude freely from
the transcript and avoid tangling with the DNA. Localised centres
of transcription may associate with other specialised complexes, such
as specific transcription factors and splicing components. Indeed,
a great deal of research has gone into testing these predictions;
many of the proteins involved in capping, splicing and 3\textquoteright{}
processing of nascent mRNA, processes that take place co-transcriptionally,
have been shown to interact with the carboxy-terminal domain (CTD)
of RNA Polymerase II {[}20{]}, prompting one author to suggest that
transcription factories should be renamed \textquoteleft{}mRNA factories\textquoteright{}
{[}21{]}. Transcription factory specialisation may have further implications
in the epigenetic control of gene expression: if transcription of
a gene only occurs within a limited number of factories in the nucleus,
the epigenetic control of chromatin mobility may affect the ability
of that gene to relocate to its transcription factory. 
\end{comment}
...
\subsubsection{Double strand breaks}


\subsubsection{Aberrant religation}
